...
module.exports = function () {
  ...
  let lightnessCurve = [4, 20, 35, 65, 90, 99]; //This is the lightness values for each tint
  let pantonedLch = [];
  let pantonedHex = [];
  for (let i = 0; i < colorBaseLch.length; i++) {
    ...
    let pantonedHexSet = [];
    let deltaL = [];
    lightnessCurve.forEach((val) => {
      //Each tint have the same Chroma and Hue. Only Lightness change
      pantonedLchSet.push({
        l: val,
        c: colorBaseLch[i].c,
        h: colorBaseLch[i].h,
    ...
    pantonedLchSet[5].c = 0.2;
    pantonedLchSet[0].c *= 0.4;
  ...
  for (let i = 0; i < colorNames.length; i++) {
    result[i] = {
      year: colorYears[i],
      name: colorNames[i],
      baseHex: colorBaseHex[i],
      baseLch: colorBaseLch[i],
      pantonedHex: pantonedHex[i],
      pantonedLch: pantonedLch[i],
    };
  }
  return result;
};
